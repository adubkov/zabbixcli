...
class ZabbixObject(object):
  ...
  def _func(self, req):
    ...
    result = None
    if self.template_id:
      obj_id = self.zapi.get_id(self.obj_type, self.obj['name'], hostid = self.template_id)
    if obj_id:
    ...
    result = "self.zapi.{obj_type}.{zbx_method}".format(obj_type=self.obj_type, zbx_method=zbx_method)
